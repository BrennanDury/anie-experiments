...
def train_fn(config, ds):
    ...
    N, H, W, Q = cfg["N"], cfg["H"], cfg["W"], cfg["Q"]
    ...
    if cfg["positional_encoding"] == "coordinate":
        pos_enc = CoordinateEncoding(torch.Size([time_width, Hp, Wp]))
        pos_unenc = CoordinateUnencoding(torch.Size([time_width, Hp, Wp]))
        ...
        pos_enc = LearnedEncoding(torch.Size([time_width, Hp, Wp, cfg["encoder_output_dim"]]))
